...
def get_xs_uu(e, y, costheta, phi_h, phi_r, lmax=2, asyms_name='sgasyms', asym_idx=0, cosine_sine_names=[]):
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(0, l+1):
            ...
            # asym_formula= f"cos({m}*({phi_h}-{phi_r}))*({asyms_name}[{asym_idx}]+{e}*{asyms_name}[{asym_idx+1}])" if m!=0 else f"({asyms_name}[{asym_idx}]+{e}*{asyms_name}[{asym_idx+1}])"
            # asym_idx += 2
            asym_formula = f"cos({m}*({phi_h}-{phi_r}))" if m!=0 else "CONSTANT"
            
            if not np.any([asym_formula in el for el in asym_formulas[depol_a]]):
                asym_formula += f"*{asyms_name}[{asym_idx}]"
                asym_idx += 1
                xs_uu_a += asym_formula
                asym_formulas[depol_a].append(asym_formula)
                asym_name = f"A_UU,T^[ cos({m}({phi_h}-{phi_r})) ]" if m!=0 else f"A_UU,T^[ Constant ]"
                asyms.append(asym_name)
            # asym_name = f"A_UU,L^[ cos({m}({phi_h}-{phi_r})) ]" if m!=0 else f"A_UU,L^[ ]"
            # asyms.append(asym_name)
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_uu_b != "":
                xs_uu_b += "+"
            asym_formula = f"cos({m}*{phi_r})" if m==2 \
                            else f"cos({2-m}*{phi_h})" if m==0 \
                            else f"cos({2-m}*{phi_h}+{m}*{phi_r})"

            if not np.any([asym_formula in el for el in asym_formulas[depol_b]]):
                asym_formula += f"*{asyms_name}[{asym_idx}]"
                asym_idx += 1
                xs_uu_b += asym_formula
                asym_formulas[depol_b].append(asym_formula)
                asym_name = f"A_UU^[ cos({m} {phi_r}) ]" if m==2 \
                            else f"A_UU^[ cos({2-m} {phi_h}) ]" if m==0 \
                            else f"A_UU^[ cos({2-m} {phi_h} + {m} {phi_r}) ]"
                asyms.append(asym_name)
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_uu_v != "":
                xs_uu_v += "+"
            asym_formula = f"cos({m}*{phi_r})" if m==1 \
                            else f"cos({1-m}*{phi_h})" if m==0 \
                            else f"cos({1-m}*{phi_h}+{m}*{phi_r})"

            if not np.any([asym_formula in el for el in asym_formulas[depol_v]]):
                asym_formula += f"*{asyms_name}[{asym_idx}]"
                asym_idx += 1
                xs_uu_v += asym_formula
                asym_formulas[depol_v].append(asym_formula)
                asym_name = f"A_UU^[ cos({m}{phi_r}) ]" if m==1 \
                            else f"A_UU^[ cos({1-m}{phi_h}) ]" if m==0 \
                            else f"A_UU^[ cos({1-m}{phi_h}+{m}{phi_r}) ]"
                asyms.append(asym_name)
    ...
    # Clean up formulas
    xs_uu = xs_uu.replace("CONSTANT","1").replace("--","+").replace("-+","-").replace("+-","-").replace("-1*","-").replace("+1*","+").replace("(1*","(").replace("*1*","*")
    for key in asym_formulas:
        for idx in range(len(asym_formulas[key])):
            asym_formulas[key][idx] = asym_formulas[key][idx].replace("CONSTANT","1").replace("--","+").replace("-+","-").replace("+-","-").replace("-1*","-").replace("+1*","+").replace("(1*","(").replace("*1*","*")
...
def get_xs_lu(e, y, costheta, phi_h, phi_r, lmax=2, asyms_name='sgasyms', asym_idx=0, cosine_sine_names=[]):
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(1, l+1): #NOTE: Start at m=1 because sin(0)=0
            ...
            if xs_lu_c != "":
                xs_lu_c += "+"
            #NOTE: This is what you would use except that the F_XX,L terms are all zero. See: http://arxiv.org/abs/1408.5721.
            # asym_formula = f"sin({m}*({phi_h}-{phi_r}))*2*({asyms_name}[{asym_idx}]+{e}*{asyms_name}[{asym_idx+1}])"
            # asym_idx += 2
            asym_formula = f"sin({m}*({phi_h}-{phi_r}))"

            if not np.any([asym_formula in el for el in asym_formulas[depol_c]]):
                asym_formula += f"*{asyms_name}[{asym_idx}]"
                asym_idx += 1
                xs_lu_c += asym_formula
                asym_formulas[depol_c].append(asym_formula)
                asyms.append(f"A_LU,T^[ sin({m}({phi_h}-{phi_r})) ]")
                # asyms.append(f"A_LU,L^[ sin({m}({phi_h}-{phi_r})) ]")
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_w]]):
                asym_formula += f"*{asyms_name}[{asym_idx}]"
                asym_idx += 1
                xs_lu_w += asym_formula
                asym_formulas[depol_w].append(asym_formula)
                asym_name = f"A_LU^[ cos({m}{phi_r}) ]" if m==1 \
                        else f"A_LU^[ cos({1-m}{phi_h}) ]" if m==0 \
                        else f"A_LU^[ cos({1-m}{phi_h}+{m}{phi_r}) ]"
                asyms.append(asym_name)
...
def get_xs_ul(e, y, costheta, phi_h, phi_r, lmax=2, asyms_name='sgasyms', asym_idx=0, cosine_sine_names=[]):
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(1, l+1): #NOTE: Start at m=1 because sin(0)=0
            ...
            if xs_ul_a != "":
                xs_ul_a += "+"
            asym_formula = f"sin(-{m}*{phi_h}+{m}*{phi_r})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_a]]):
                ...
                asym_formulas[depol_a].append(asym_formula)
                asyms.append(f"A_UL^[ sin(-{m}{phi_h}+{m}{phi_r}) ]")
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_ul_b != "":
                xs_ul_b += "+"
            asym_formula = f"sin({m}*{phi_r})" if m==2 \
                        else f"sin({2-m}*{phi_h})" if m==0 \
                        else f"sin({2-m}*{phi_h}+{m}*{phi_r})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_b]]):
                ...
                asym_formulas[depol_b].append(asym_formula)
                asym_name = f"A_UL^[ sin({m}{phi_r}) ]" if m==2 \
                        else f"A_UL^[ sin({2-m}{phi_h}) ]" if m==0 \
                        else f"A_UL^[ sin({2-m}{phi_h}+{m}{phi_r}) ]"
                asyms.append(asym_name)
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_ul_v != "":
                xs_ul_v += "+"
            asym_formula = f"sin({m}*{phi_r})" if m==1 \
                        else f"sin({1-m}*{phi_h})" if m==0 \
                        else f"sin({1-m}*{phi_h}+{m}*{phi_r})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_v]]):
                ...
                asym_formulas[depol_v].append(asym_formula)
                asym_name = f"A_UL^[ sin({m}{phi_r}) ]" if m==1 \
                        else f"A_UL^[ sin({1-m}{phi_h}) ]" if m==0 \
                        else f"A_UL^[ sin({1-m}{phi_h}+{m}{phi_r}) ]"
                asyms.append(asym_name)
...
def get_xs_ll(e, y, costheta, phi_h, phi_r, lmax=2, asyms_name='sgasyms', asym_idx=0, cosine_sine_names=[]):
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(0, l+1):
            ...
            if xs_ll_c != "":
                xs_ll_c += "+"
            asym_formula = f"cos({m}*({phi_h}-{phi_r}))" if m!=0 else f"CONSTANT"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_c]]):
                ...
                asym_formulas[depol_c].append(asym_formula)
                asym_name = f"A_LL^[ cos({m}({phi_h}-{phi_r})) ]" if m!=0 else f"A_LL^[ Constant ]"
                asyms.append(asym_name)
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_w]]):
                ...
                asym_formulas[depol_w].append(asym_formula)
                asym_name = f"A_LL^[ cos({m}{phi_r}) ]" if m==1 \
                        else f"A_LL^[ cos({1-m}{phi_h}) ]" if m==0 \
                        else f"A_LL^[ cos({1-m}{phi_h}+{m}{phi_r}) ]"
                asyms.append(asym_name)
    ...
    # Clean up formulas
    xs_ll = xs_ll.replace("CONSTANT","1").replace("--","+").replace("-+","-").replace("+-","-").replace("-1*","-").replace("+1*","+").replace("(1*","(").replace("*1*","*")
    for key in asym_formulas:
        for idx in range(len(asym_formulas[key])):
            asym_formulas[key][idx] = asym_formulas[key][idx].replace("CONSTANT","1").replace("--","+").replace("-+","-").replace("+-","-").replace("-1*","-").replace("+1*","+").replace("(1*","(").replace("*1*","*")
    for idx in range(len(asyms)):
...
def get_xs_ut(e, y, costheta, phi_h, phi_r, phi_s, lmax=2, asyms_name='sgasyms', asym_idx=0, cosine_sine_names=[]):
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_ut_a != "":
                xs_ut_a += "+"
            #NOTE: This is what you would use except that the F_XX,L terms are all zero. See: http://arxiv.org/abs/1408.5721.
            # asym_formula = f"sin({m+1}*{phi_h}-{phi_s})*({asyms_name}[{asym_idx}]+{e}*{asyms_name}[{asym_idx+1}])" if m==0 \
            #             else f"sin({m}*{phi_r}-{phi_s})*({asyms_name}[{asym_idx}]+{e}*{asyms_name}[{asym_idx+1}])" if m==-1 \
            #             else f"sin({m+1}*{phi_h}-{m}*{phi_r}-{phi_s})*({asyms_name}[{asym_idx}]+{e}*{asyms_name}[{asym_idx+1}])"
            # asym_idx += 2
            asym_formula = f"sin({m+1}*{phi_h}-{phi_s})" if m==0 \
                        else f"sin({m}*{phi_r}-{phi_s})" if m==-1 \
                        else f"sin({m+1}*{phi_h}-{m}*{phi_r}-{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_a]]):
                ...
                asym_formulas[depol_a].append(asym_formula)
                asym_name = f"A_UT,T^[ sin({m+1}{phi_h}-{phi_s}) ]" if m==0 \
                        else f"A_UT,T^[ sin({m}{phi_r}-{phi_s}) ]" if m==-1 \
                        else f"A_UT,T^[ sin({m+1}{phi_h}-{m}{phi_r}-{phi_s}) ]"
                asyms.append(asym_name)
                # asym_name = f"A_UT,L^[ sin({m+1}{phi_h}-{phi_s}) ]" if m==0 \
                #         else f"A_UT,L^[ sin({m}{phi_r}-{phi_s}) ]" if m==-1 \
                #         else f"A_UT,L^[ sin({m+1}{phi_h}-{m}{phi_r}-{phi_s}) ]"
                # asyms.append(asym_name)
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_ut_b != "":
                xs_ut_b += "+"
            asym_formula = f"sin({m}*{phi_r}+{phi_s})" if m==1 \
                        else f"sin({1-m}*{phi_h}+{phi_s})" if m==0 \
                        else f"sin({1-m}*{phi_h}+{m}*{phi_r}+{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_b]]):
                ...
                asym_formulas[depol_b].append(asym_formula)
                asym_name = f"A_UT^[ sin({m}{phi_r}+{phi_s}) ]" if m==1 \
                        else f"A_UT^[ sin({1-m}{phi_h}+{phi_s}) ]" if m==0 \
                        else f"A_UT^[ sin({1-m}{phi_h}+{m}{phi_r}+{phi_s}) ]"
                asyms.append(asym_name)

            # And second term
            if xs_ut_b != "":
                xs_ut_b += "+"
            asym_formula = f"sin({m}*{phi_r}-{phi_s})" if m==3 \
                        else f"sin({3-m}*{phi_h}-{phi_s})" if m==0 \
                        else f"sin({3-m}*{phi_h}+{m}*{phi_r}-{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_b]]):
                ...
                xs_ut_b += asym_formula
                asym_formulas[depol_b].append(asym_formula)
                asym_name = f"A_UT^[ sin({m}{phi_r}-{phi_s}) ]" if m==3 \
                        else f"A_UT^[ sin({3-m}{phi_h}-{phi_s}) ]" if m==0 \
                        else f"A_UT^[ sin({3-m}{phi_h}+{m}{phi_r}-{phi_s}) ]"
                asyms.append(asym_name)
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_ut_v != "":
                xs_ut_v += "+"
            asym_formula = f"sin({phi_s})" if m==0 \
                        else f"sin(-{m}*{phi_h}+{m}*{phi_r}+{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_v]]):
                ...
                asym_formulas[depol_v].append(asym_formula)
                asym_name = f"A_UT^[ sin({phi_s}) ]" if m==0 \
                        else f"A_UT^[ sin(-{m}{phi_h}+{m}{phi_r}+{phi_s}) ]"
                asyms.append(asym_name)

            # And second term
            if xs_ut_v != "":
                xs_ut_v += "+"
            asym_formula = f"sin({m}*{phi_r}-{phi_s})" if m==2 \
                        else f"sin({2-m}*{phi_h}-{phi_s})" if m==0 \
                        else f"sin({2-m}*{phi_h}+{m}*{phi_r}-{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_v]]):
                ...
                xs_ut_v += asym_formula
                asym_formulas[depol_v].append(asym_formula)
                asym_name = f"A_UT^[ sin({m}{phi_r}-{phi_s}) ]" if m==2 \
                        else f"A_UT^[ sin({2-m}{phi_h}-{phi_s}) ]" if m==0 \
                        else f"A_UT^[ sin({2-m}{phi_h}+{m}{phi_r}-{phi_s}) ]"
                asyms.append(asym_name)
...
def get_xs_lt(e, y, costheta, phi_h, phi_r, phi_s, lmax=2, asyms_name='sgasyms', asym_idx=0, cosine_sine_names=[]):
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(0, l+1):
            ...
            if xs_lt_c != "":
                xs_lt_c += "+"
            asym_formula = f"cos({m}*{phi_r}-{phi_s})" if m==1 \
                            else f"cos({1-m}*{phi_h}-{phi_s})" if m==0 \
                            else f"cos({1-m}*{phi_h}+{m}*{phi_r}-{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_c]]):
                ...
                asym_formulas[depol_c].append(asym_formula)
                asym_name = f"A_LT^[ cos({m}{phi_r}-{phi_s}) ]" if m==1 \
                            else f"A_LT^[ cos({1-m}{phi_h}-{phi_s}) ]" if m==0 \
                            else f"A_LT^[ cos({1-m}{phi_h}+{m}{phi_r}-{phi_s}) ]"
                asyms.append(asym_name)
    ...
    for l in range(0,lmax+1):
        ...
        for m in range(-l, l+1):
            ...
            if xs_lt_w != "":
                xs_lt_w += "+"
            asym_formula = f"cos({phi_s})" if m==0 \
                            else f"cos(-{m}*{phi_h}+{m}*{phi_r}+{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_w]]):
                ...
                asym_formulas[depol_w].append(asym_formula)
                asym_name = f"A_LT^[ cos({phi_s}) ]" if m==0 \
                            else f"A_LT^[ cos(-{m}{phi_h}+{m}{phi_r}+{phi_s}) ]"
                asyms.append(asym_name)

            # And second term
            if xs_lt_w != "":
                xs_lt_w += "+"
            asym_formula = f"cos({m}*{phi_r}-{phi_s})" if m==2 \
                            else f"cos({2-m}*{phi_h}-{phi_s})" if m==0 \
                            else f"cos({2-m}*{phi_h}+{m}*{phi_r}-{phi_s})"
            ...
            if not np.any([asym_formula in el for el in asym_formulas[depol_w]]):
                ...
                xs_lt_w += asym_formula
                asym_formulas[depol_w].append(asym_formula)
                asym_name = f"A_LT^[ cos({m}{phi_r}-{phi_s}) ]" if m==2 \
                            else f"A_LT^[ cos({2-m}{phi_h}-{phi_s}) ]" if m==0 \
                            else f"A_LT^[ cos({2-m}{phi_h}+{m}{phi_r}-{phi_s}) ]"
                asyms.append(asym_name)
...
nfitvars_rgh = 3
e = f'x[{len(asyms_ut)+len(asyms_lt)+nfitvars_rgh}]' #epsilon #epsilon_mc
y = 'y' #y #y_mc
costheta = 'cos(theta_p1_pipim)' #'cos(x[0])'
phi_h = 'x[0]' # phi_h_pipim
phi_r = 'x[1]' # phi_rt_pipim
