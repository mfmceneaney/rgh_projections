    ...
    for ch in chs:
        ...
        csv_path = os.path.abspath(
            os.path.join(
                ...
                f'jobs/saga/test_getBinKinematics/test_getBinKinematics__{ch}/out_{rg}_1d_bins_binscheme_kinematics.csv'
            )
        )
        hist_path = os.path.abspath(
            os.path.join(
                RGH_PROJECTIONS_HOME,
                f'jobs/saga/test_getBinKinematicsTH2Ds__{ch}/out_{rg}_1d_bins_binscheme_kinematics.root'
            )
        )
        ...
        if ch=='pipim':
            xlabels[f'mass_{ch}'] = '$M_{'+ch_labels[ch]+'}$ (GeV)'
        xlims = {
            f'mass_{ch}':[0.0,3.0],
