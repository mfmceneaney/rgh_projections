...
echo EXECUTING: RGH_GEMC_COMMAND \"$GCARD -SAVE_ALL_MOTHERS=1 -SKIPREJECTEDHITS=1 -NGENP=50 -INTEGRATEDRAW=\\\"*\\\" -USE_GUI=0 -RUNNO=11 -INPUT_GEN_FILE=\\\"LUND, $LUNDFILE\\\" -OUTPUT=\\\"hipo, $GEMCFILE\\\" -N=$NEVENTS\"
RGH_GEMC_COMMAND "$GCARD -SAVE_ALL_MOTHERS=1 -SKIPREJECTEDHITS=1 -NGENP=50 -INTEGRATEDRAW=\"*\" -USE_GUI=0 -RUNNO=11 -INPUT_GEN_FILE=\"LUND, $LUNDFILE\" -OUTPUT=\"hipo, $GEMCFILE\" -N=$NEVENTS"
...
echo EXECUTING: RGH_RECON_UTIL_COMMAND -i $GEMCFILE -o $RECFILE -n $NEVENTS -y $YAML
RGH_RECON_UTIL_COMMAND "-i $GEMCFILE -o $RECFILE -n $NEVENTS -y $YAML"
...
$RGH_HIPO_UTILS_COMMAND "-filter -b \"$FILTER_BANKS\" -merge -o $DSTFILE $RECFILE"
